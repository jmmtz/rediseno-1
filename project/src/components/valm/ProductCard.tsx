...
export default function ProductCard({ product, onAddToCart }: ProductCardProps) {
  ...
  return (
    <div className="group bg-[#111] border border-white/10 hover:border-white/30 rounded-2xl overflow-hidden transition-all duration-500 hover:shadow-[0_8px_32px_rgba(0,0,0,0.5)] flex flex-col">
      ...
      <div className="relative overflow-hidden bg-[#2B2823] aspect-[3/4]">
        <img
        ...
        />
        {isOnSale && (
          <div className="absolute top-3 left-3 bg-white text-[#2B2823] text-xs font-black px-2.5 py-1 rounded-sm tracking-wider flex items-center gap-1">
            <Tag size={10} />
            SALE -{discountPercent}%
          </div>
        )}
        <div className="absolute top-3 right-3 bg-[#2B2823]/80 text-white text-[10px] font-medium px-2 py-1 rounded tracking-widest">
          VALM
        </div>
      </div>
              ...
              {product.shades.map((shade) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded text-xs transition-all duration-200 border ${
                    selectedShade === shade
                      ? 'border-white bg-white text-[#2B2823] font-medium'
        ...
        <button
          ...
          className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-semibold tracking-wide transition-all duration-300 ${
            added
              ...
              : 'bg-white hover:bg-gray-100 text-[#2B2823] active:scale-95'
          }`}
        >
          <ShoppingBag size={16} />
