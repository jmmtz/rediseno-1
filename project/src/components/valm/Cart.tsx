...
export default function Cart({ items, onClose, onUpdateQty, onRemove, onCheckout }: CartProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      <div className="flex-1 bg-[#2B2823]/60 backdrop-blur-sm" onClick={onClose} />
      <div className="w-full max-w-md bg-[#2B2823] border-l border-white/10 flex flex-col h-full shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/10">
          <div className="flex items-center gap-2">
            ...
            {items.length > 0 && (
              <span className="bg-white text-[#2B2823] text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">
        ...
        {items.length > 0 && (
          <div className="border-t border-white/10 px-6 py-5 space-y-3">
            ...
            <button
              onClick={onCheckout}
              className="w-full bg-white hover:bg-gray-100 text-[#2B2823] font-semibold py-4 rounded-xl transition-all duration-200 active:scale-95 text-sm tracking-wide"
            >
