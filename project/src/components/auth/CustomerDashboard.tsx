...
export default function CustomerDashboard({ userId, userEmail, userName, onClose, onLogout, onBookNow }: CustomerDashboardProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#2B2823]/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-[#FBFBF9] w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl shadow-2xl border border-gray-100 max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-[#F0E8DA] flex items-center justify-center text-[#8A7A5C] font-bold text-sm shrink-0">
        ...
        <div className="px-5 pt-4 pb-1 shrink-0">
          <p className="text-gray-900 font-semibold text-sm flex items-center gap-2">
            <Calendar size={16} className="text-[#8A7A5C]" />
            Mis Citas
            {upcoming.length > 0 && (
              <span className="text-xs px-1.5 py-0.5 rounded-full font-bold bg-[#2B2823] text-white">
                {upcoming.length}
              </span>
            )}
          </p>
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-5">
          {loading ? (
            <div className="flex items-center justify-center py-16">
              <div className="w-7 h-7 border-2 border-[#2B2823]/30 border-t-[#2B2823] rounded-full animate-spin" />
            </div>
          ) : (
              ...
              {appointments.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <button
                    ...
                    className="bg-[#2B2823] hover:bg-[#3A362F] text-white text-sm font-semibold px-5 py-2.5 rounded-2xl transition-colors"
                  >
                    Agendar Cita
                  </button>
                </div>
    ...
    <div className="bg-[#FBFBF9] border border-gray-100 rounded-xl p-4">
      ...
      <div className="flex items-center gap-4 mt-2.5">
        ...
        {appt.payment_amount > 0 && (
          <span className="text-[#8A7A5C] text-xs font-medium ml-auto">Anticipo: ${appt.payment_amount.toLocaleString()}</span>
        )}
      </div>
    </div>
  );
