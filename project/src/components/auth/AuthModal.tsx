...
export default function AuthModal({ initialView = 'login', onClose, onAuthSuccess }: AuthModalProps) {
  ...
  const inputCls = "w-full bg-[#FBFBF9] border border-gray-200 rounded-xl px-4 py-3 text-gray-900 placeholder-gray-400 focus:border-[#2B2823] focus:ring-2 focus:ring-[#2B2823]/20 focus:outline-none text-sm transition-all";
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#2B2823]/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-[#FBFBF9] rounded-2xl w-full max-w-sm shadow-2xl border border-gray-100 overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-6 pb-4">
          <img src="/IMG_4562.PNG" alt="La Rue" className="h-8 w-auto object-contain" />
          ...
          {view === 'login' ? (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-[#2B2823] hover:bg-[#3A362F] disabled:opacity-50 text-white font-semibold py-3.5 rounded-2xl transition-all flex items-center justify-center gap-2 text-sm mt-2"
              >
                {loading && <div className="w-4 h-4 border-2 border-[#2B2823]/30 border-t-[#2B2823] rounded-full animate-spin" />}
                {loading ? 'Ingresando...' : 'Iniciar Sesión'}
              </button>

              <p className="text-center text-xs text-gray-400">
                ¿No tienes cuenta?{' '}
                <button type="button" onClick={() => { setView('signup'); setError(''); }} className="text-[#8A7A5C] font-medium hover:underline">
                  Regístrate
                </button>
              </p>
            </form>
            ...
            <form onSubmit={handleSignUp} className="space-y-4">
              ...
              <p className="text-center text-xs text-gray-400">
                ¿Ya tienes cuenta?{' '}
                <button type="button" onClick={() => { setView('login'); setError(''); }} className="text-[#8A7A5C] font-medium hover:underline">
                  Inicia sesión
                </button>
              </p>
            </form>
