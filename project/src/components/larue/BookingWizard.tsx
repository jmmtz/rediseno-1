          ...
          return (
            <button
              ...
              className={`mx-auto w-9 h-9 rounded-full text-sm font-medium flex items-center justify-center transition-all duration-150
                ${isSelected ? 'bg-[#2B2823] text-white font-bold shadow-sm'
                  : isToday && !past ? 'bg-[#F0E8DA] text-[#8A7A5C] font-semibold border border-[#2B2823]'
                  : past ? 'text-gray-300 cursor-not-allowed'
                  : 'text-gray-700 hover:bg-[#F0E8DA] hover:text-[#8A7A5C]'}`}
            >
              {day}
            </button>
          );
...
export default function BookingWizard({ onClose, preselectedService, customerSession }: BookingWizardProps) {
  ...
  const inputCls = "w-full bg-[#FBFBF9] border border-gray-200 rounded-xl px-4 py-3 text-gray-900 placeholder-gray-400 focus:border-[#2B2823] focus:ring-2 focus:ring-[#2B2823]/20 focus:outline-none transition-all text-sm";
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#2B2823]/50 backdrop-blur-sm flex items-center justify-center p-4">
        ...
        {step !== 'confirm' && (
          <div className="px-6 pt-5 pb-2 flex gap-1.5">
            {(['datetime','service','stylist','info','payment'] as Step[]).map((s, i) => (
              <div key={s} className={`h-1 flex-1 rounded-full transition-all duration-300 ${i <= currentIndex ? 'bg-[#2B2823]' : 'bg-gray-100'}`} />
            ))}
          </div>
        )}
        ...
        <div className="px-6 py-6">
            ...
            <div className="space-y-5">
              ...
              {selectedDate && (
                <div className="flex items-center gap-2 px-1">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#2B2823]" />
                  <p className="text-xs text-[#8A7A5C] font-medium capitalize">{formatSelectedDate(selectedDate)}</p>
                </div>
              )}
              {selectedDate && (
                <div>
                  <p className="text-sm font-semibold text-gray-800 mb-3 flex items-center gap-2">
                    <Clock size={14} className="text-[#8A7A5C]" /> Selecciona un horario
                  </p>
                    ...
                    {TIME_SLOTS.map((time) => (
                      <button
                        ...
                        className={`py-2.5 rounded-xl text-sm font-medium transition-all duration-150 ${
                          selectedTime === time
                            ? 'bg-[#2B2823] text-white shadow-sm font-bold'
                            : 'bg-[#FBFBF9] border border-gray-200 text-gray-600 hover:border-[#2B2823] hover:bg-[#F0E8DA]'
                        }`}
                      >
                        {time}
                      </button>
              ...
              <button
                ...
                className="w-full bg-[#2B2823] hover:bg-[#3A362F] disabled:bg-gray-100 disabled:text-gray-400 text-white font-semibold py-3.5 rounded-2xl transition-all duration-200"
              >
                Continuar
              </button>
            </div>
          ...
          {step === 'service' && (
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full bg-[#2B2823] hover:bg-[#3A362F] disabled:bg-gray-100 disabled:text-gray-400 text-white font-semibold py-3.5 rounded-2xl transition-all"
              >
                Continuar
              </button>
            </div>
          ...
          {step === 'stylist' && (
            <div className="space-y-3">
              ...
              <button
                ...
                className={`w-full text-left px-4 py-3.5 rounded-xl border-2 transition-all duration-200 ${
                  anyProfessional ? 'border-[#2B2823] bg-[#F0E8DA]' : 'border-gray-100 bg-[#FBFBF9] hover:border-gray-200'
                }`}
              >
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-[#2B2823]/20 flex items-center justify-center">
                    <User size={16} className="text-[#8A7A5C]" />
                  </div>
                  <div>
          ...
          {step === 'info' && (
            <div className="space-y-4">
              ...
              {saved && (
                <div className="bg-[#F0E8DA] border border-[#2B2823]/30 rounded-xl px-4 py-2.5 text-xs text-[#8A7A5C]">
          ...
          {step === 'payment' && (
            <div className="space-y-5">
              ...
              {requiresPayment && (
                <div className="space-y-3">
                  <p className="text-gray-500 text-xs flex items-center gap-1.5">
                    <CreditCard size={13} className="text-[#8A7A5C]" />
              ...
              <button
                ...
                className="w-full bg-[#2B2823] hover:bg-[#3A362F] disabled:opacity-50 text-white font-semibold py-3.5 rounded-2xl transition-all flex items-center justify-center gap-2"
              >
                {paymentLoading
                  ? <><div className="w-4 h-4 border-2 border-[#2B2823]/30 border-t-[#2B2823] rounded-full animate-spin" /> Procesando...</>
          ...
          {step === 'confirm' && (
            <div className="text-center space-y-6 py-4">
              <div className="flex justify-center">
                <div className="w-20 h-20 rounded-full bg-[#F0E8DA] flex items-center justify-center">
                  <CheckCircle size={40} className="text-[#8A7A5C]" />
                </div>
              </div>
              <div>
              ...
              <button
                onClick={onClose}
                className="w-full bg-[#2B2823] hover:bg-[#3A362F] text-white font-semibold py-3.5 rounded-2xl transition-colors"
              >
                Cerrar
              </button>
            </div>
